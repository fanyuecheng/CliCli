...
#import "CCFloatLayoutView.h"
#import "CCUIDefines.h"

#define ValueSwitchAlignLeftOrRight(valueLeft, valueRight) ([self shouldAlignRight] ? valueRight : valueLeft)

const CGSize CCUIFloatLayoutViewAutomaticalMaximumItemSize = {-1, -1};

@implementation CCFloatLayoutView
...
- (void)didInitialize {
    ...
    self.maximumItemSize = CCUIFloatLayoutViewAutomaticalMaximumItemSize;
}
...
- (CGSize)layoutSubviewsWithSize:(CGSize)size shouldLayout:(BOOL)shouldLayout {
    ...
    CGSize maximumItemSize = CGSizeEqualToSize(self.maximumItemSize, CCUIFloatLayoutViewAutomaticalMaximumItemSize) ? CGSizeMake(size.width - UIEdgeInsetsGetHorizontalValue(self.padding), size.height - UIEdgeInsetsGetVerticalValue(self.padding)) : self.maximumItemSize;
    NSInteger line = -1;
